...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // ...
	...
	auto C1 = root->addChild(8);
	root->addChild(15);
	auto C11 = C1->addChild(16);
	C1->addChild(23);
	C11->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<typename GenericTree<T>::TreeNode*> q;
  q.push(rootNodePtr);
  while(!q.empty()){
  	auto node = q.front();
  	q.pop();
  	results.push_back(node->data);
  	for(int i = 0; i < node->childrenPtrs.size(); i++) q.push(node->childrenPtrs[i]);
  }
