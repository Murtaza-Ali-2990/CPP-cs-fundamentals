...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // TODO: Your code here!
	...
	if(head_ == nullptr){
		...
		Node* temp = head_;
		while(temp->next != nullptr && newData > temp->next->data) temp = temp->next;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node* left_p = head_;
  Node* right_p = right.getHeadPtr();

  while(left_p != nullptr && right_p != nullptr){
  	if(left_p->data <= right_p->data){
  		merged.pushBack(left_p->data);
  		left_p = left_p->next;
  	} else {
  		merged.pushBack(right_p->data);
  		right_p = right_p->next;
  	}
  }

  while(left_p != nullptr){
  	merged.pushBack(left_p->data);
  	left_p = left_p->next;
  }
